...
def movie_predictor():
    ...
    put_text("")
    put_link("80's", "https://movie-predictor-00s-yxpwksz6pa-ue.a.run.app", new_window = True)
    put_text("")
    put_link("2010's", "https://movie-predictor-10s-7sycrvxa2a-ue.a.run.app", new_window = True)
    put_text("© 2022 Vincent Yabor")
    
    data = input_group("You can either input your own plot or the title of a 80's movie", [textarea("Movie plot summary goes here! (25 character minimum)", required = False, name = "Plot"), input("Or type in a 80's movie title! (Case sensitive - must match IMDb title)", name = "Title", required = False)])
    ...
    if((data["Plot"] != "") and (data["Title"] == "")):
        ...
        if names == []:
            ...
            popup("Predictions", [put_text("Genres that fit your plot:"), put_table(tdata = result.values.tolist(),header = ['Predicted Genre', 'Predicted Genre Score (Out of 1)']), put_text("Here are some 80's movies that fit your plot:"), put_table(tdata = similar_movies.values.tolist(), header = ['Similar Movies', 'Genres', 'Year', 'Rating'])], closable = True)
            actions(buttons=["Refresh the page"])
            pywebio.session.run_js('window.location.reload()')
            return(result, similar_movies)
        ...
        if(movie_ratings[movie_ratings['primaryTitle'] == title].shape[0] > 1):
            ...
            popup(f"Similar 80's Movies to {data['Title']}:", put_table(tdata = similar_movies.values.tolist(), header = ['Similar Movies', 'Genres', 'Year', 'Rating']), closable = True)
            actions(buttons=["Refresh the page"])
            pywebio.session.run_js('window.location.reload()')
