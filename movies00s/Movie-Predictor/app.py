...
def movie_predictor():
    ...
    data = input_group("You can either input your own plot or the title of a 2000's movie", [textarea("Movie plot summary goes here! (25 character minimum)", required = False, name = "Plot"), input("Or type in a 2000's movie title! (Case sensitive - must match IMDb title)", name = "Title", required = False)])
    ...
    if((data["Plot"] != "") and (data["Title"] == "")):
        ...
        if names == []:
            ...
            popup("Predictions", [put_text("Genres that fit your plot:"), put_table(tdata = result.values.tolist(),header = ['Predicted Genre', 'Predicted Genre Score (Out of 1)']), put_text("Here are some 2000's movies that fit your plot:"), put_table(tdata = similar_movies.values.tolist(), header = ['Similar Movies', 'Genres', 'Year', 'Rating'])], closable = True)
            actions(buttons=["Refresh the page"])
            pywebio.session.run_js('window.location.reload()')
            return(result, similar_movies)
        ...
        if(movie_ratings[movie_ratings['primaryTitle'] == title].shape[0] > 1):
            ...
            popup(f"Similar 2000's Movies to {data['Title']}:", put_table(tdata = similar_movies.values.tolist(), header = ['Similar Movies', 'Genres', 'Year', 'Rating']), closable = True)
            actions(buttons=["Refresh the page"])
            pywebio.session.run_js('window.location.reload()')
