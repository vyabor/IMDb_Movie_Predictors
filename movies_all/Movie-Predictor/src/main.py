...
def movie_predictor():
    ...
    #titles.append(" ")
    data = input_group("You can either input your own plot or the title of a movie", [textarea("Movie plot summary goes here! (25 character minimum)", required = False, name = "Plot"), input("Or type in a movie title! (Case sensitive - must match IMDb title)", name = "Title", required = False)])
    #data = input_group("You can either input your own plot or the title of a movie", [textarea("Movie plot summary goes here! (25 character minimum)", required = False, name = "Plot"), select("Or select a movie title!", name = "Title", required = False, options = np.sort(titles), help_text = "Hint: Try typing to search for a movie", value = " ")])
